...
//Page I used for my 404 error response: https://github.com/h5bp/html5-boilerplate/blob/master/src/404.html
...
int directoryCheck(char * filename){
	struct stat filenameStat;
	stat(filename, &filenameStat);
	return S_ISDIR(filenameStat.st_mode);
}
...
void serve_request(int client_fd){
  ...
  struct dirent * request_next = NULL;
  while(1){
  ...
  strncpy(&filename[1],requested_file,4095);
	if (directoryCheck(filename)){
		sprintf(temp, "%s%s" ,filename, "index.html");
		if (access(temp, F_OK) == -1){
			DIR * request_dir = opendir(filename);
			snprintf(temp, sizeof(temp), index_hdr, filename, filename);
			send(client_fd, request_str, strlen(request_str), 0);
			send(client_fd, temp, strlen(temp), 0);
			if(request_dir != NULL) {
				memset(temp, 0, sizeof(temp));
				while((request_next = readdir(request_dir)) != NULL) {
					snprintf(temp, sizeof(temp), index_body, request_next->d_name, request_next->d_name);
					send(client_fd, temp, strlen(temp), 0);
				}
	...
	else if((read_fd = open(filename,0,0)) != -1){
		if (strstr(filename, ".html")){
			...
			send(client_fd, request_ico, strlen(request_ico), 0);
		}		
			...
			send(client_fd, request_text, strlen(request_text), 0);
		}	
		...
		send(client_fd, request_404, strlen(request_404), 0);
		send(client_fd, message_404, strlen(message_404), 0);
		}	
  close(read_fd);
  close(client_fd);
  return;
...
int main(int argc, char** argv) {
    ...
    while(1) {
		...
		/* Allows the server to access threads to allow multiple client connections
		 */
		pthread_t threads;
		pthread_create(&threads, NULL, thread, (void *) sock);
